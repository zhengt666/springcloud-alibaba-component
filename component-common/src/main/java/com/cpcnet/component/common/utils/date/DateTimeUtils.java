package com.cpcnet.component.common.utils.date;
...
public class DateTimeUtils {
    ...
    /**
     * String（yyyy-MM-dd HH:mm:ss）转LocalDateTime
     *
     * @param formatTime 格式化时间字符串
     * @return LocalDateTime时间
     */
    public static LocalDateTime getLocalDateTime(String formatTime) {
        return getLocalDateTime(formatTime, 0);
    }
    ...
    /**
     * 将LocalDateTime时间转为Long型的时间戳
     *
     * @param localDateTime
     * @return
     */
    public static long toTimestampLong(LocalDateTime localDateTime) {
        ZonedDateTime zonedDateTime = localDateTime
                .atZone(TimeZone.getTimeZone(DEFAULT_TIME_ZONE).toZoneId());
     ...
     * 根据字符串格式获取时间格式
     *
     * @param formatTime 字符串格式
     * @param state      是最早还是最晚 0最早，1最晚
     * @return LocalDateTime 时间格式
     */
    public static LocalDateTime getLocalDateTime(String formatTime, Integer state) {
        //yyyy-MM-dd HH:mm:ss.SSS
        if (Pattern.matches("^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}.\\d{3}$", formatTime)) {
            return LocalDateTime.parse(formatTime, DateTimeFormatter.ofPattern(FORMAT_DATE_TIME_MS));
        }
        //yyyyMMddHHmmss
        if (Pattern.matches("^\\d{4}\\d{2}\\d{2}\\d{2}\\d{2}\\d{2}$", formatTime)) {
            return LocalDateTime.parse(formatTime, DateTimeFormatter.ofPattern(PURE_FORMAT_DATE_TIME));
        }
        //yyyy-MM-dd HH:mm
        if (Pattern.matches("^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}$", formatTime)) {
            if (state.equals(0)) {
                formatTime += ":00";
            } else {
                formatTime += ":59";
        ...
        //yyyy-MM-dd
        if (Pattern.matches("^\\d{4}-\\d{2}-\\d{2}$", formatTime)) {
            if (state.equals(0)) {
                formatTime += DATE_HMS_STR;
            } else {
                formatTime += DATE_MAX_HMS_STR;
            }
        }
        //yyyy-MM-dd HH:mm:ss
        if (Pattern.matches("^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}$", formatTime)) {
            return LocalDateTime.parse(formatTime, DateTimeFormatter.ofPattern(FORMAT_DATE_TIME));
        }
        throw new RuntimeException("未知时间格式！");
    }
    
    /**
     * 根据字符串格式获取日期格式（yyyy-MM-dd）
     *
     * @param formatTime 字符串格式
     * @return LocalDate 日期格式（yyyy-MM-dd）
     */
    public static LocalDate getLocalDate(String formatTime) {
        //yyyy-MM-dd
        if (Pattern.matches("^\\d{4}-\\d{2}-\\d{2}$", formatTime)) {
            formatTime += DATE_HMS_STR;
            return LocalDateTime.parse(formatTime).toLocalDate();
        } else {
            throw new RuntimeException("未知时间格式！");
        }
    }
     ...
     * 根据字符串格式获取时间（yyyy-MM）
     *
     * @param formatTime 字符串格式 yyyy-MM
     * @return 返回当月第一天开始时间 如输入：2021-06，输出2021-06-01 00:00:00
     */
    public static LocalDateTime getFromYearAndMonth(String formatTime) {
        //yyyy-MM
        if (Pattern.matches("^\\d{4}-\\d{2}$", formatTime)) {
            // 补充日期和时间
            formatTime += "-01" + DATE_HMS_STR;
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
            return LocalDateTime.parse(formatTime, formatter);
        } else {
            throw new RuntimeException("未知时间格式！");
        }
    }
    ...
    public static LocalDateTime min(LocalDateTime localDateTime1, LocalDateTime localDateTime2) {
        if (null == localDateTime1 || null == localDateTime2) {
            throw new RuntimeException("无法比较值为null的时间日期");
        }
    ...
    public static boolean isSameDay(LocalDateTime localDateTime1, LocalDateTime localDateTime2) {
        ...
        if (formatter.format(localDateTime1).equals(formatter.format(localDateTime2))) {
            return true;
        }
        return false;
    }
